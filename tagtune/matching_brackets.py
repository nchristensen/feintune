...
def matching_brackets(string, idx, opening_bracket="[", closing_bracket="]"):
    if idx < len(string) and string[idx] == "[":
        opening = [i for i, c in enumerate(string[idx + 1 :]) if c == opening_bracket]
        closing = [i for i, c in enumerate(string[idx + 1 :]) if c == closing_bracket]
...
def matching_brackets_dict(string, opening_bracket="[", closing_bracket="]"):
    ...
    dc = {
        ...
        (c==opening_bracket and op.append(i) and False) or (c==closing_bracket and op)
    }
