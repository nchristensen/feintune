...
pids = [file[:-6] for file in files if not (str(file).endswith(
    "full.hjson") or str(file).endswith("default.hjson"))]
...
faster_df_only_tuned_better = pd.DataFrame(
    data=faster_dict_list_only_tuned_better).fillna(0)
default_df_only_worse = pd.DataFrame(
    data=default_dict_list_only_worse).fillna(0)
faster_df_only_default_exists = pd.DataFrame(
    data=faster_dict_only_default_exists).fillna(0)
...
avg_roofline_full = average(
    full_df["frac_roofline_flop_rate"], weights=full_execution_times)
avg_roofline_default = average(
    default_df["frac_roofline_flop_rate"], weights=default_df["avg_time"])
avg_roofline_predicted = average(
    batch_df["frac_roofline_flop_rate"], weights=batch_df["avg_time"])
avg_roofline_faster = average(
    faster_df["frac_roofline_flop_rate"], weights=faster_df["avg_time"])
avg_roofline_faster_only_tuned = average(
    faster_df_only_tuned["frac_roofline_flop_rate"], weights=faster_df_only_tuned["avg_time"])
# , weights=faster_df_only_tuned_better["avg_time"])
avg_roofline_faster_only_tuned_better = average(
    faster_df_only_tuned_better["frac_roofline_flop_rate"])
# , weights=default_df_only_worse["avg_time"])
avg_roofline_default_only_worse = average(
    default_df_only_worse["frac_roofline_flop_rate"])
...
default_df = default_df.assign(weighted_avg_time=pd.Series(
    get_weighted_avg_time(default_df)).values)
faster_df = faster_df.assign(weighted_avg_time=pd.Series(
    get_weighted_avg_time(faster_df)).values)
...
default_df_only_worse = default_df_only_worse.assign(
    weighted_avg_time=pd.Series(get_weighted_avg_time(default_df_only_worse)).values)


default_only_worse_avg_time = np.sum(
    default_df_only_worse["weighted_avg_time"])
faster_only_better_avg_time_total = np.sum(
    faster_df_only_tuned_better["weighted_avg_time"])

speedup2 = default_only_worse_avg_time / faster_only_better_avg_time_total

tuning_potential2 = faster_df_only_tuned_better["weighted_avg_time"].to_numpy(
)*(1 - faster_df_only_tuned_better["frac_roofline_flop_rate"])
remaining_speedup2 = faster_only_better_avg_time_total / \
    (faster_only_better_avg_time_total - np.sum(tuning_potential2))


print("All kernels", np.sum(
    default_df["weighted_avg_time"]), faster_avg_time_total, speedup, remaining_speedup)
print("Sped up kernels", np.sum(
    default_df_only_worse["weighted_avg_time"]), faster_only_better_avg_time_total, speedup2, remaining_speedup2)
...
# """
plt.semilogy(sorted(faster_df["avg_time"],
             reverse=True), label="Autotuned transformations")
plt.semilogy(sorted(default_df["avg_time"],
             reverse=True), label="Handtuned transformations")
