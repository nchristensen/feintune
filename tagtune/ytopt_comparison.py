...
data = load_hjson(base_directory + "/four_axis_hjson_dummy" +
                  "/" + kernel_id + ".hjson")["data"]
roofline_flop_rate = data["roofline_flop_rate"]
flops = data["flops"]

plt.figure()

labels = {"rf": "Random Forest", "dummy": "Random sampling",
          "gbrt": "Gradient Boosted Regression Trees"}


for strategy in strategies:

    csv_filename = base_directory + "/four_axis_hjson_" + \
        strategy + "/" + kernel_id + ".csv"
    dataframe = read_csv(csv_filename)["objective"].cummin()
    print(dataframe)
    plt.plot(np.arange(1, len(dataframe[:]) + 1), flops /
             dataframe[:]/roofline_flop_rate, label=labels[strategy])

plt.plot(np.arange(1, len(dataframe[:] + 1)), 0.5168756271127658 *
         np.ones((100,)), "k--", label="Exhaustive search maximum")
